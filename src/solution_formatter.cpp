...
std::string SolutionFormatter::format(const std::vector<uint8_t>& solution, 
    ...
    logger.debug("Formatting solution (" + std::to_string(solution.size()) + " bytes)");
    
    switch (format) {
        case ASSEMBLY:
            logger.debug("Output format: Assembly");
            return disassemble(solution);
        case BINARY:
            logger.debug("Output format: Binary");
            return formatBytes(solution);
        case C_ARRAY:
            logger.debug("Output format: C Array");
            return formatCArray(solution);
        case BASIC_DATA:
            logger.debug("Output format: BASIC DATA");
            return formatBasicData(solution);
        default:
            logger.debug("Output format: Default (Assembly)");
            return disassemble(solution);
    }
}
...
std::string SolutionFormatter::getStatistics(const std::vector<uint8_t>& solution) {
    Logger& logger = getLogger();
    logger.debug("Calculating statistics for solution");
    
    size_t size = calculateSize(solution);
    size_t cycles = calculateCycles(solution);
    
    // Calculate additional statistics for debug output
    logger.debug("Solution size: " + std::to_string(size) + " bytes");
    logger.debug("Estimated cycle count: " + std::to_string(cycles));
    
    // Count instruction types for debug info
    std::map<std::string, int> instruction_types;
    size_t pos = 0;
    while (pos < solution.size()) {
        ...
        if (info_it != opcode_info.end()) {
            instruction_types[info_it->second.mnemonic]++;
            pos += info_it->second.bytes;
        } else {
            // Unknown opcode
            pos++;
        }
    ...
    for (const auto& [mnemonic, count] : instruction_types) {
        logger.debug("  " + mnemonic + ": " + std::to_string(count));
    }
...
std::string SolutionFormatter::disassemble(const std::vector<uint8_t>& bytes) {
    Logger& logger = getLogger();
    logger.debug("Disassembling " + std::to_string(bytes.size()) + " bytes");
    ...
    logger.debug("Disassembly complete, generated " + std::to_string(oss.str().size()) + " characters");
    return oss.str();
}

std::string SolutionFormatter::formatBytes(const std::vector<uint8_t>& bytes) {
    Logger& logger = getLogger();
    logger.debug("Formatting " + std::to_string(bytes.size()) + " bytes as hex dump");
...
std::string SolutionFormatter::formatCArray(const std::vector<uint8_t>& bytes) {
    Logger& logger = getLogger();
    logger.debug("Formatting " + std::to_string(bytes.size()) + " bytes as C array");
...
std::string SolutionFormatter::formatBasicData(const std::vector<uint8_t>& bytes) {
    Logger& logger = getLogger();
    logger.debug("Formatting " + std::to_string(bytes.size()) + " bytes as BASIC DATA statements");
