...
namespace phaistos {
    ...
    void TransformationCache<AddressT>::add(
        ...
        if (it == cache.end()) {
            ...
            entry.cycle_optimal_cycles = cycles;  // Store cycles correctly
            ...
            if (cycles < it->second.cycle_optimal_cycles) {  // Compare with cycles, not sequence size
                it->second.cycle_optimal_sequence = sequence;
                it->second.cycle_optimal_cycles = cycles;  // Store cycles correctly
            }
        }
    }
